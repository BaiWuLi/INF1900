/**
 * Programme utilisé dans notre classe Communication
 * dans le fichier communication.h pour l'adapter à nos besoins.
 * \file memoire_24.h
 */
...
public:
    ...
    static uint8_t lecture(const uint16_t adresse, uint8_t* donnee);
    ...
    //une donnee a la fois
    static uint8_t ecriture(const uint16_t adresse, const uint8_t donnee);
    //bloc de donnees : longueur doit etre de 127 et moins
    static uint8_t
...
private:
    //pour l'ecriture
    static uint8_t ecrire_page(const uint16_t adresse,
                               uint8_t*       donnee,
...
private:
    ...
    static const uint8_t PAGE_SIZE = 128;
};
